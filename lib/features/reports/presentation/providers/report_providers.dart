...
final reportByIdProvider = FutureProvider.family<Report?, String>((ref, id) async {
  ...
  if (reports != null) {
    for (final report in reports) { if (report.id == id) return report; }
  }
...
class SubmitReportController extends AsyncNotifier<Report?> {
  ...
  Future<Report?> submit({required String title, required String description, required ReportCategory category, required List<String> photos, double? latitude, double? longitude, String? address}) async {
    state = const AsyncValue.loading();
    final user = ref.read(currentUserProvider);
    final now = DateTime.now();
    final report = Report(
      id: const Uuid().v4(), title: title, description: description, category: category,
      status: ReportStatus.submitted, createdAt: now, photos: photos, latitude: latitude,
      longitude: longitude, address: address, userId: user?.uid ?? 'me',
      timeline: [TimelineEntry(status: ReportStatus.submitted, date: now)],
    );
    final result = await AsyncValue.guard(() => ref.read(reportRepositoryProvider).submitReport(report));
    state = result;
    return result.valueOrNull;
  }
}

final submitReportControllerProvider = AsyncNotifierProvider<SubmitReportController, Report?>(SubmitReportController.new);
